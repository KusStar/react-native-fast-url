...
namespace fasturl {
            ...
            return HOST_FN(rt, "forEach", 1, {
                ...
                while (keys.has_next()) {
                    ...
                    if (key.has_value()) {
                        ...
                        jsi::String jsiValueStr = jsi::String::createFromUtf8(
                            rt, std::string(value_view));
                        jsi::String jsiKeyStr = jsi::String::createFromUtf8(
                            rt, std::string(key_str));

                        callback.asFunction(rt).call(rt, jsiValueStr,
                                                     jsiKeyStr);
                    }
                }
                return {};
        ...
        if (key == "get") {
            return HOST_FN(rt, "get", 1, {
                if (count != 1) {
                    throw jsi::JSError(
                        rt,
                        "Failed to execute 'get' on 'URLSearchParams': get "
                        "accepts 1 string argumement");
                }
                // NOTE: Technically browsers allow URLSearchParams to accept
                // non-string keys but for our implementation we only allow
                // strings
                if (!args[0].isString()) {
    ...
    void URLHostObject::set(jsi::Runtime &rt, const jsi::PropNameID &name,
